...
Run against `cotique.Quartz.Spi.MongoDbJobStore` **2.1.0-rc.1**, Quartz 3.19.1,
...
- `DisallowConcurrentExecution` is not free: it is what converts a dead pod into a stopped
  schedule. Here it stays on regardless: two instances running one slot would each write the
  same record. The cost is accepted, not overlooked, and the mitigation is the monitoring
  below rather than removing the attribute.
...
`docker network disconnect` on the pod that was executing, 2.2.0, same bench. The process stays alive, keeps its CPU, and its database calls start failing.
