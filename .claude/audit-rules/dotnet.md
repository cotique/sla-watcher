...
> A rule stays `[predicted]` until it catches a real defect. When one does, retag it
> `[observed]` and add the one-line story.
>
...
- [observed] [judgment] **`[DisallowConcurrentExecution]` is not free, but the cost is now a
  ...
  ever reclaimed that: measured on the bench, the live pod sat idle for four and a half minutes
  ...
  Keep it where an overlap would corrupt shared state. Full account in
...
- [observed] [mechanical] **A write that reports failure may have been applied.** Measured on
  the bench: a frozen and thawed pod's insert threw `MongoConnectionException` /
