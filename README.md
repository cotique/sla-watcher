...
A .NET 9 worker that reads a ticket tracker on a schedule, measures how long tickets sat in a
status **in the reviewer's working hours**, and raises an escalation when one leaves the band.

The service is deterministic. There is no model inside it, deliberately: `DECISIONS.md` says
why, and that document is authoritative for every "why is it like this" question this README
does not answer.

**Status: first slice, in progress.** The scheduler, the store integration and the failure
behaviour are built and measured. The tracker client exists; nothing reads a real tracker yet.
...
Ekaterina Kozhina, sole maintainer. There is no tracker for this project's own work, which is a
property of a local experiment and transfers nowhere.

## Where things are written down

| File | Holds |
|---|---|
| `DECISIONS.md` | every decision and the reason it was forced. Authoritative |
